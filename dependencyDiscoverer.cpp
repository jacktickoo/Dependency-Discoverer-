...
struct table {
public:
    std::unordered_map<std::string, std::list<std::string>> tableMap;
    std::mutex mutex;
    //need to rename these functions and function calls
    auto findT(std::string name){
    ...
    void insertT(const std::pair <std::string, std::list<std::string>>& data){
        std::unique_lock <std::mutex> lock(mutex);
        tableMap.insert(data);
    };
};
...
public:
    ...
    std::optional <std::string> pop() {
        ...
        c.wait(lock, [this] { return !workQList.empty();});

        auto v = workQList.front();
        workQList.pop_front();
        return v;
    }
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (theTable.findT(name) != theTable.endT()) { continue; }
    // ... insert mapping from file name to empty list in table ...
    theTable.insertT( { name, {} } );
    // ... append file name to workQ
    workQ.push( name );
  }
  // 3. close file
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = &(theTable.tableMap)[name];
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    theTable.insertT( { obj, { argv[i] } } );
    
    // 3b. insert mapping from file.ext to empty list
    theTable.insertT( { argv[i], { } } );
  ...
  if ( getenv("CRAWLER_THREADS") != NULL){
      crawlerThreads = std::stoi(getenv("CRAWLER_THREADS"));
  }
    std::thread ts[crawlerThreads];
  ...
  for(int i = 0; i < crawlerThreads; i++){
      ...
      ts[i] = std::thread([] {
          ...
          while (workQ.size() > 0) {
              ...
              if (theTable.findT(filename) == theTable.endT()) {
                  fprintf(stderr, "Mismatch between table and workQ\n");
              }
              else {
                  process(filename.c_str(), &(theTable.tableMap)[filename]);
              }
          }
      });
